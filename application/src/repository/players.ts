...
export async function patchPlayer(player: IPlayer, playerData: Pick<IPlayer, 'blacklistedPlayerId'>) {
  await databaseClient
    .db('matchmaking')
    .collection('players')
    .updateOne({ discordId: player.discordId, guildId: player.guildId }, { $set: { blacklistedPlayerId: playerData.blacklistedPlayerId } });
}

export async function createPlayer(playerData: IPlayer) {
  const operationResult = await databaseClient
    .db('matchmaking')
    .collection('players')
    .insertOne({ ...playerData });
...
export async function retrieveLobbyPlayers(playerIds: Array<string>, guildId: string): Promise<Array<IPlayer>> {
  const cursor = databaseClient
    .db('matchmaking')
    .collection('players')
    .find<IPlayer>({ discordId: { $in: playerIds }, guildId: { $eq: guildId } });
...
export async function updateRating(player: IPlayer) {
  try {
    await databaseClient
      .db('matchmaking')
      .collection('players')
      .updateOne({ discordId: player.discordId, guildId: player.guildId }, { $set: { rating: player.rating } });
  } catch (error) {
...
export async function getTopRated(guildId: string) {
  const cursor = databaseClient
    .db('matchmaking')
    .collection('players')
    .find({ guildId }, { limit: 10, sort: { rating: 'desc' } });
  const players = await cursor.toArray();
  return players;
}
